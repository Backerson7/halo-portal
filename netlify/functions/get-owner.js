...
exports.handler = async (event) => {
    ...
    if (!token) {
          return { statusCode: 400, headers, body: JSON.stringify({ error: 'Missing owner token' }) };
    }
    ...
    try {
      ...
      if (data.object === 'error') {
              return { statusCode: 500, headers, body: JSON.stringify({ error: data.message, code: data.code, db_id: NOTION_DB_ID }) };
      }

      if (!data.results || data.results.length === 0) {
              const totalResults = data.results ? data.results.length : 'null';
              return { statusCode: 404, headers, body: JSON.stringify({ error: 'Owner not found', searched_token: token, total_results: totalResults, db_id: NOTION_DB_ID }) };
      }
      ...
      return { statusCode: 200, headers, body: JSON.stringify(owner) };
